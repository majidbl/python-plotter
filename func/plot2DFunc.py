import matplotlib.pyplot as plt
from .fileExtract import *
import gi
import os
import numpy as np
gi.require_version('Gtk', '3.0')
from gi.repository import Gtk


class plot2DF():

    def __init__(self):
        self.File_Save_Name = "test"
        self.spin_button_digits = 1000
        self.Format_Save = "eps"
        self.Paths = os.path.expanduser("~")
        # self.Paths = "/home/majid"
        self.Type_Save = "Line"

    def on_format_ComboBox_Changed(self, combo):
        tree_iter = combo.get_active_iter()
        if tree_iter is not None:
            model = combo.get_model()
            self.Format_Save = model[tree_iter][0]
            print(self.Format_Save)
        # return self.Format_Save

    def on_type_ComboBox_Changed(self, combo):
        tree_iter = combo.get_active_iter()
        if tree_iter is not None:
            model = combo.get_model()
            self.Type_Save = model[tree_iter][0]
            print(self.Type_Save)
        # return self.Format_Save

    def on_spin_btn_Changed(self, spinbutton):
        self.spin_button_digits = spinbutton.get_value_as_int()
        print(self.spin_button_digits)
        # return spin_button_digits

    def on_d_save_file_name_Entry_Changed(self, entry):
        self.File_Save_Name = entry.get_text()
        print(self.File_Save_Name)
        # return File_Save_Name

    def plotting(self, inputList, stateP, locs):
        if stateP == "SubPlot-X":
            fx, axarr = plt.subplots(len(inputList), sharex=True)
        if stateP == "SubPlot-Y":
            fy, ayarr = plt.subplots(len(inputList), sharey=True)
        for i in range(len(inputList)):
            fex = dataExtract()
            fex.getData2d(inputList[i][0], inputList[i][7])
            X = fex.x
            Y = fex.y
            if stateP == "Seperate":
                plt.figure()
                plt.plot(X, Y, marker=inputList[i][6],
                         linestyle=inputList[i][1], color=inputList[i][5],
                         label=inputList[i][2])
                plt.xlabel(inputList[i][3])
                plt.ylabel(inputList[i][4])
                plt.legend(loc=locs)
            if stateP == "Together":
                plt.plot(X, Y, marker=inputList[i][6],
                         linestyle=inputList[i][1], color=inputList[i][5],
                         label=inputList[i][2])
                plt.xlabel(inputList[0][3])
                plt.ylabel(inputList[0][4])
                plt.legend(loc=locs)
            if stateP == "SubPlot-X":
                axarr[i].plot(X, Y, marker=inputList[i][6],
                              linestyle=inputList[i][1],
                              color=inputList[i][5],
                              label=inputList[i][2])
                axarr[i].legend(loc=locs)
            if stateP == "SubPlot-Y":
                ayarr[i].plot(X, Y, marker=inputList[i][6],
                              linestyle=inputList[i][1],
                              color=inputList[i][5],
                              label=inputList[i][2])
                axarr[i].legend(loc=locs)
        plt.show()

    def plot_Save(self, inputList, stateP, locs):
        winSave = Gtk.Window()
        d_format_Label = Gtk.Label("format Save : ")
        d_format_Store = Gtk.ListStore(str)
        formats = ["None", "eps", "jpeg", "jpg", "pdf", "pgf", "png", "ps",
                   "raw", "rgba", "svg", "svgz", "tif", "tiff"]
        for d_format in formats:
            d_format_Store.append([d_format])
        d_format_ComboBox = Gtk.ComboBox.new_with_model(d_format_Store)
        d_format_ComboBox.connect("changed",
                                  self.on_format_ComboBox_Changed)
        renderer_text = Gtk.CellRendererText()
        d_format_ComboBox.pack_start(renderer_text, True)
        d_format_ComboBox.add_attribute(renderer_text, "text", 0)
        d_spin_btn_Label = Gtk.Label("DPI : ")
        adjustment = Gtk.Adjustment(1000, 0, 10000, 1, 10, 0)
        d_spin_btn = Gtk.SpinButton()
        d_spin_btn.set_adjustment(adjustment)
        d_spin_btn.connect("changed",
                           self.on_spin_btn_Changed)
        d_save_fle_name_Label = Gtk.Label("File Name : ")
        d_save_file_name_Entry = Gtk.Entry()
        d_save_file_name_Entry.connect("changed",
                                       self.on_d_save_file_name_Entry_Changed)
        d_folder_btn_Label = Gtk.Label("Path To Save : ")
        d_folder_btn = Gtk.Button("Select Folder")
        d_folder_btn.connect("clicked", self.on_folder_clicked)
        save_ok_btn = Gtk.Button("Ok")
        save_ok_btn.connect("clicked", self.on_save_ok_Button_clicked, winSave,
                            inputList, stateP, locs)
        save_cancel_btn = Gtk.Button("Cancel")
        save_cancel_btn.connect("clicked", self.on_save_cancel_Button_clicked,
                                winSave)
        d_grid = Gtk.Grid()
        d_grid.set_border_width(20)
        d_grid.set_column_spacing(10)
        d_grid.set_row_spacing(10)
        d_grid.attach(d_format_Label, 0, 0, 1, 1)
        d_grid.attach(d_format_ComboBox, 1, 0, 1, 1)
        d_grid.attach(d_spin_btn_Label, 0, 1, 1, 1)
        d_grid.attach(d_spin_btn, 1, 1, 1, 1)
        d_grid.attach(d_save_fle_name_Label, 0, 2, 1, 1)
        d_grid.attach(d_save_file_name_Entry, 1, 2, 1, 1)
        d_grid.attach(d_folder_btn_Label, 0, 3, 1, 1)
        d_grid.attach(d_folder_btn, 1, 3, 1, 1)
        d_grid.attach(d_folder_btn_Label, 0, 3, 1, 1)
        d_grid.attach(d_folder_btn, 1, 3, 1, 1)
        d_grid.attach(save_ok_btn, 0, 4, 1, 1)
        d_grid.attach(save_cancel_btn, 1, 4, 1, 1)
        d_grid.show_all()
        winSave.add(d_grid)
        winSave.show()

    def on_save_ok_Button_clicked(self, bottun, window, inputL, stateP, locs):
        window.destroy()
        if stateP == "SubPlot-X":
            fx, axarr = plt.subplots(len(inputL), sharex=True)
        if stateP == "SubPlot-Y":
            fy, ayarr = plt.subplots(len(inputL), sharey=True)
        for i in range(len(inputL)):
            fex = dataExtract()
            fex.getData2d(inputL[i][0])
            X = fex.x
            Y = fex.y
            if stateP == "Seperate":
                plt.figure()
                plt.plot(X, Y, marker=inputL[i][6],
                         linestyle=inputL[i][1],
                         color=inputL[i][5],
                         label=inputL[i][2])
                plt.xlabel(inputL[i][3])
                plt.ylabel(inputL[i][4])
                plt.legend(loc=locs)
                pathSaveS = self.Paths + "/" + self.File_Save_Name + "_" + str(i + 1) + "." + self.Format_Save
                plt.savefig(pathSaveS, format=self.Format_Save,
                            dpi=self.spin_button_digits)
            if stateP == "Together":
                plt.plot(X, Y, marker=inputL[i][6],
                         linestyle=inputL[i][1],
                         color=inputL[i][5],
                         label=inputL[i][2])
                plt.xlabel(inputL[0][3])
                plt.ylabel(inputL[0][4])
                plt.legend(loc=locs)
                pathSave = self.Paths + "/" + self.File_Save_Name + "." + self.Format_Save
                plt.savefig(pathSave, format=self.Format_Save,
                            dpi=self.spin_button_digits)
            if stateP == "SubPlot-X":
                axarr[i].plot(X, Y, marker=inputL[i][6],
                              linestyle=inputL[i][1],
                              color=inputL[i][5],
                              label=inputL[i][2])
                axarr[i].legend(loc=locs)
            if stateP == "SubPlot-Y":
                ayarr[i].plot(X, Y, marker=inputL[i][6],
                              linestyle=inputL[i][1],
                              color=inputL[i][5],
                              label=inputL[i][2])
                axarr[i].legend(loc=locs)
            if stateP == "SubPlot-X":
                pathSaveSx = self.Paths + "/" + self.File_Save_Name + "." + self.Format_Save
                fx.savefig(pathSaveSx, format=self.Format_Save,
                           dpi=self.spin_button_digits)
            if stateP == "SubPlot-Y":
                pathSaveSy = self.Paths + "/" + self.File_Save_Name+ "." + self.Format_Save
                fy.savefig(pathSaveSy, format=self.Format_Save,
                           dpi=self.spin_button_digits)
            dialog = Gtk.MessageDialog(None, 0, Gtk.MessageType.QUESTION,
                                       Gtk.ButtonsType.CLOSE,
                                       "Save Fig(s) !")
            response = dialog.run()
            if response == Gtk.ResponseType.CLOSE:
                    dialog.destroy()

    def on_save_cancel_Button_clicked(self, bottun, window):
        window.destroy()

    def on_folder_clicked(self, widget):
        dialog = Gtk.FileChooserDialog("Please choose a folder", None,
                                       Gtk.FileChooserAction.SELECT_FOLDER,
                                       (Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL,
                                        "Select", Gtk.ResponseType.OK))
        dialog.set_default_size(800, 400)

        response = dialog.run()
        if response == Gtk.ResponseType.OK:
            print("Select clicked")
            print("Folder selected: " + dialog.get_filename())
            self.Paths = dialog.get_filename()
        elif response == Gtk.ResponseType.CANCEL:
            print("Cancel clicked")
        dialog.destroy()
